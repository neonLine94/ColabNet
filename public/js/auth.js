// document.addEventListener('DOMContentLoaded', () => {
    

//     // Initiale Benutzer erstellen, wenn keine vorhanden sind
//     if (!localStorage.getItem('users')) {
//         const initialUsers = [
//             { username: 'admin', password: 'admin', role: 'admin', company: 'Admin', firstName: 'Admin', lastName: 'User', position: 'Admin', phone: '123456789', email: 'admin@example.com' },
//             { username: 'user', password: 'user', role: 'user', company: 'User Company', firstName: 'User', lastName: 'User', position: 'Technician', phone: '987654321', email: 'user@example.com' }
//         ];
//         localStorage.setItem('users', JSON.stringify(initialUsers));
//     }

//     // Anmeldefunktion
//     const loginForm = document.getElementById('loginForm');
//     if (loginForm) {
//         loginForm.addEventListener('submit', (event) => {
//             event.preventDefault();
//             const username = document.getElementById('username').value;
//             const password = document.getElementById('password').value;

//             const users = JSON.parse(localStorage.getItem('users')) || [];
//             const user = users.find(u => u.username === username && u.password === password);

//             if (user) {
//                 localStorage.setItem('user', JSON.stringify(user));
//                 window.location.href = 'dashboard.html';
//             } else {
//                 alert('Ungültige Anmeldedaten');
//             }
//         });
//     }

//     // Logout-Funktion
//     const logoutButton = document.getElementById('logoutButton');
//     if (logoutButton) {
//         logoutButton.addEventListener('click', () => {
//             fetch('/backend/logout.php') // Ruft das PHP-Logout-Skript auf
//                 .then(() => {
//                     window.location.href = 'login.html'; // Umleitung nach dem Logout
//                 });
//         });
//     }
// });


// MySQL

document.addEventListener('DOMContentLoaded', () => {
    // // Initiale Benutzer erstellen, wenn keine vorhanden sind (für Testzwecke)
    // if (!localStorage.getItem('users')) {
    //     const initialUsers = [
    //         { username: 'admin', password: 'admin', role: 'admin', company: 'Admin', firstName: 'Admin', lastName: 'User', position: 'Admin', phone: '123456789', email: 'admin@example.com' },
    //         { username: 'user', password: 'user', role: 'user', company: 'User Company', firstName: 'User', lastName: 'User', position: 'Technician', phone: '987654321', email: 'user@example.com' }
    //     ];
    //     localStorage.setItem('users', JSON.stringify(initialUsers));
    // }

    // Login-Formular Verifikation
    const loginForm = document.getElementById('loginForm');
    if (loginForm) {
        loginForm.addEventListener('submit', (event) => {
            event.preventDefault();
            const username = document.getElementById('username').value;
            const password = document.getElementById('password').value;

            // Sende die Anmeldedaten an das PHP-Skript zur Überprüfung
            fetch('backend/login.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: `username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`
            })
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
                    // Erfolgreiche Anmeldung, Umleitung zum Dashboard
                    window.location.href = 'dashboard.html';
                } else {
                    // Fehlermeldung anzeigen, wenn die Anmeldedaten falsch sind
                    alert('Ungültige Anmeldedaten');
                }
            })
            .catch(error => console.error('Fehler bei der Anmeldung:', error));
        });
        }

    // Logout-Funktion
    const logoutButton = document.getElementById('logoutButton');
    if (logoutButton) {
        logoutButton.addEventListener('click', () => {
            fetch('/backend/logout.php') // PHP-Logout aufrufen
                .then(() => {
                    window.location.href = 'login.html'; // Umleitung nach Logout
                });
        });
    }
});
